...
if __name__ == "__main__":
    textPath = os.path.join(os.path.dirname(
        os.path.abspath(__file__)), 'articles.txt')
    corpus_path = os.path.join(os.path.dirname(
        os.path.abspath(__file__)), 'corpus_train.txt')
    cluster_keyword_lda = os.path.join(os.path.dirname(
        os.path.abspath(__file__)), 'cluster_keywords_lda.txt')
    cluster_keyword_lsi = os.path.join(os.path.dirname(
        os.path.abspath(__file__)), 'cluster_keywords_lsi.txt')
    sentence_dict, dictionary, corpus, corpus_tfidf = create_data(corpus_path)
    lsi_model(sentence_dict, dictionary, corpus,
              corpus_tfidf, cluster_keyword_lsi)
    lda_model(sentence_dict, dictionary, corpus,
              corpus_tfidf, cluster_keyword_lda)
